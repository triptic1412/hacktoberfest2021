...
bool wildcardMatching(string pattern, string text)
{
    ...
    //base case
    //if both the strings are empty - hence they match
    if(m==0 and n==0){
        return true;
    }
    
    else if(n==0){        //if pattern is empty - we can't match anything with characters left in text string
        return false;
    }
    else if(m==0){         //if text is empty and only '*' is present (even more than 1), then only pattern and text can match
    	bool found = false;
    	for(int i=0;i<n;i++){
    		if(pattern[i]!='*'){
    			found = true;
